 ...
 */
ngx_int_t
ngx_event_timer_init(event_timer_t** ev_timer, zlog_category_t *zlog_handler)
{
    *ev_timer = (event_timer_t*)malloc(sizeof(event_timer_t));
    ...
    timer->node_num = 0;
    timer->log_handler = zlog_handler;
    return 0;
}
...
ngx_event_expire_timers(event_timer_t* ev_timer)
{
    ...
    for ( ;; ) {
        ...
        zlog_info(ev_timer->log_handler, "event timer del ev->timer.key: %lu\n", ev->timer.key);
        pthread_mutex_lock(&(ev_timer->mutex));
